...
class Model(object):
    ...
    def get_equal_fpr_constraints(self, constraints_slack=1.0):
        ...
        constraints_list = [(tf.losses.hinge_loss(self.labels_placeholder, self.predictions_tensor, weights=tf.multiply(protected_placeholder, rbc_utils.flip_binary_tensor(self.labels_placeholder)), reduction=tf.compat.v1.losses.Reduction.MEAN) - average_fpr 
    ...
    def get_equal_fpr_constraints_nonconvex(self, constraints_slack=0.0):
        average_fpr = tf.losses.hinge_loss(self.labels_placeholder, self.predictions_tensor, weights=rbc_utils.flip_binary_tensor(self.labels_placeholder), reduction=tf.compat.v1.losses.Reduction.MEAN)
        constraints_list = [(tf.losses.hinge_loss(self.labels_placeholder, self.predictions_tensor, weights=tf.multiply(protected_placeholder, rbc_utils.flip_binary_tensor(self.labels_placeholder)), reduction=tf.compat.v1.losses.Reduction.MEAN) - average_fpr 
                            - (constraints_slack * tf.ones_like(average_fpr))) for protected_placeholder in self.protected_placeholders]
        return constraints_list
    ...
    def get_equal_tpr_constraints(self, constraints_slack=0.0):
        ...
        constraints_list = [(average_tpr - losses.concave_hinge_loss(rbc_utils.flip_binary_tensor(self.labels_placeholder), self.predictions_tensor, weights=tf.multiply(protected_placeholder, self.labels_placeholder), reduction=tf.compat.v1.losses.Reduction.MEAN) 
    ...
    def get_equal_tpr_constraints_nonconvex(self, constraints_slack=0.0):
        average_tpr = tf.losses.hinge_loss(rbc_utils.flip_binary_tensor(self.labels_placeholder), self.predictions_tensor, weights=self.labels_placeholder, reduction=tf.compat.v1.losses.Reduction.MEAN)
        constraints_list = [(average_tpr - tf.losses.hinge_loss(rbc_utils.flip_binary_tensor(self.labels_placeholder), self.predictions_tensor, weights=tf.multiply(protected_placeholder, self.labels_placeholder), reduction=tf.compat.v1.losses.Reduction.MEAN) 
                            - (constraints_slack * tf.ones_like(average_tpr))) for protected_placeholder in self.protected_placeholders]
        return constraints_list
    ...
    def get_equal_tpr_constraints_dro(self, constraints_slack=0.0):
        ...
        average_tpr = tf.losses.hinge_loss(rbc_utils.flip_binary_tensor(self.labels_placeholder), self.predictions_tensor, weights=self.labels_placeholder, reduction=tf.compat.v1.losses.Reduction.MEAN)
        constraints_list = [(average_tpr - losses.concave_hinge_loss(rbc_utils.flip_binary_tensor(
    ...
    def get_equal_fpr_constraints_dro(self, constraints_slack=0.0): # ADDED (May 07): Copied FPR code from below
            average_fpr = losses.concave_hinge_loss(self.labels_placeholder, self.predictions_tensor, weights=rbc_utils.flip_binary_tensor(self.labels_placeholder), reduction=tf.compat.v1.losses.Reduction.MEAN)
            constraints_list_fpr = [(tf.losses.hinge_loss(self.labels_placeholder, self.predictions_tensor, weights=tf.multiply(rbc_utils.flip_binary_tensor(self.labels_placeholder), tf.reshape(p_variable,(self.num_data,1))), reduction=tf.compat.v1.losses.Reduction.MEAN) - average_fpr - (constraints_slack * tf.ones_like(average_fpr))) for p_variable in self.p_variables_list]
            constraints_list = constraints_list_fpr
            return constraints_list
    ...
    def get_equal_tpr_and_fpr_constraints_dro(self, constraints_slack=0.0):
            ...
            constraints_list_tpr = [(average_tpr - losses.concave_hinge_loss(rbc_utils.flip_binary_tensor(
                self.labels_placeholder), self.predictions_tensor, 
            weights=tf.multiply(self.labels_placeholder, tf.reshape(p_variable,(self.num_data,1))), 
            reduction=tf.compat.v1.losses.Reduction.MEAN) - (constraints_slack * tf.ones_like(average_tpr))) for p_variable in self.p_variables_list]
