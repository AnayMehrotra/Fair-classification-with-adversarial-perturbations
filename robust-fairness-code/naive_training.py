    ...
    for i in range(num_runs):
        ...
        for learning_rate_theta in learning_rates_theta:
            for learning_rate_lambda in learning_rates_lambda:
                ...
                best_index_iters = rbc_utils.find_best_candidate_index(np.array(results_dict['train_01_objective_vector']),np.array(results_dict['train_01_proxy_Ghat_constraints_matrix']), rank_objectives=True, max_constraints=True)
                ...
                results_dict_best_idx = rbc_utils.add_results_dict_best_idx(results_dict, best_index_iters)
        ...
        best_index = rbc_utils.find_best_candidate_index(np.array(val_objectives),np.array(val_constraints_matrix), rank_objectives=True, max_constraints=True)
...
def train_one_model(input_df, 
    ...
    best_index_iters = rbc_utils.find_best_candidate_index(np.array(results_dict['train_01_objective_vector']),np.array(results_dict['train_01_proxy_Ghat_constraints_matrix']))
    results_dict_best_idx = rbc_utils.add_results_dict_best_idx(results_dict, best_index_iters)
