...
class _Lagrangian:
    ...
    def solve_linprog(self, nu):
        ...
        try:
            ...
            print('TypeError')
            print('res.x:', res.x)
            print('self.hs:', self.hs)
            print('self.hs.index:', self.hs.index)
...
def _mean_pred(dataX, hs, weights):
    ...
    pred = pd.DataFrame()
    #print(f'(Internal classred) # of classifiers: {len(hs)}')
...
def expgrad(dataX, dataA, dataY, learner, cons=moments.DP(), eps=0.01,
            T=50, nu=None, eta_mul=2.0, debug=True):
    """
...
#         print (Qsum.index.tolist())
        if not h_idx in Qsum.index.tolist():
            Qsum.at[h_idx] = 0.0
        Qsum[h_idx] += 1.0
    ...
    for h_idx in hs.index:
        if not h_idx in weights.index.tolist():
