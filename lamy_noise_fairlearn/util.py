...
sys.path.append(os.getcwd()+'/lamy_noise_fairlearn/fairlearn/')
import classred as red
import moments

sys.path.append(os.getcwd()+'/lamy_noise_fairlearn/fair_classification/')
import utils as ut
import funcs_disp_mist as fdm
import loss_funcs as lf

sys.path.append(os.getcwd()+'/lamy_noise_fairlearn/fairERM/')
...
def change_format_new(dataset_train, train_labels, dataset_test, test_labels, sensible_feature, include_sensible):
    '''
    Change data format into that needed for Agarwal classifier. More preprocessing will be needed if Zafar's classifier gets used.
    '''
    d_train = dict()
    ...
    for i in range(dataset_train.data.shape[1]):
        if i != sensible_feature or include_sensible:
            d_train[i] = dataset_train[:, i]
            d_test[i] = dataset_test[:, i]

    dataX = pd.DataFrame(d_train)
    dataY = pd.Series(train_labels)

    dataA = pd.Series(dataset_train[:, sensible_feature])
    dataA[dataA>0] = 1
    dataA[dataA<=0] = 0

    dataX_test = pd.DataFrame(d_test)
    dataY_test = pd.Series(test_labels)

    dataA_test = pd.Series(dataset_test[:, sensible_feature])
    dataA_test[dataA_test>0] = 1
    dataA_test[dataA_test<=0] = 0
...
def denoiseA(data_cor, rho, mode):
    ...
    cor_dataA = np.array([int(z) for z in cor_dataA.values])
    lnl.fit(X = dataX.values, s = cor_dataA, noise_matrix=noise_matrix)
...
def _run_test_Agarwal(test, data, sensible_name, learner, creteria):
    ...
    def _get_stats(clf, dataX, dataA, dataY):
        pred = clf(dataX)
        print(f'(Internal lamy) Predictions: min {np.min(pred)}, max {np.max(pred)}, mean {np.mean(pred)}')

        disp, sr, fpr, fdr, tpr = fair_measure(pred, dataA, dataY, creteria)
        #
        error = np.mean(np.abs(dataY.values-pred.values))
        return disp, sr, fpr, fdr, tpr, error

    print("here...")

    res["disp_train"], res["sr_train"], res["fpr_train"], res["fdr_train"], res["tpr_train"], res["error_train"] = _get_stats(Q, dataX_train, dataA_train, dataY_train)
    res["disp_test"], res["sr_test"], res["fpr_test"], res["fdr_test"], res["tpr_test"], res["error_test"] = _get_stats(Q, dataX_test, dataA_test, dataY_test)
    res["acc_test"] = 1 - res["error_test"]
    res["acc_train"] = 1 - res["error_train"]
...
from lamy_noise_fairlearn.util import _scale_eps, _run_test

keys = ["disp_train", "disp_test", "sr_train", "sr_test", "fpr_train", "fpr_test", "fdr_train", "fdr_test", "tpr_train", "tpr_test",\
        "error_train", "error_test", "acc_train", "acc_test"]
eps = None
rho_list = None
...
def lamy_experiment(dataset_train, train_labels, dataset_test, test_labels, tests, rho, trials, sensible_name, sensible_feature, creteria, classifier, include_sensible, learner, n, verbose):

    print('starting lamy')
    all_data = [{k:[[] for _ in range(trials)] for k in keys} for _ in range(n)]
    for i in range(trials):
        data_cor = change_format_new(dataset_train, train_labels, dataset_test, test_labels, sensible_feature, include_sensible)
        # [dataX, dataY, dataA, dataX, dataY, dataA, dataX_test, dataY_test, dataA_test]
        print('changed format')

        cor_dataA = data_cor[2]
        dataY = data_cor[1]

        # "denoises" dataset A, data_denoised_est = None and rho_est = None unless in mode 'six'
        #data_denoised, data_denoised_est, rho_est = denoiseA(data_cor, rho, mode)
        #print('denoised')
        alpha_a, beta_a = estimate_alpha_beta(cor_dataA, dataY, rho, creteria)

        print(f'Lamy internal: (\\alpha,\\beta)={alpha_a},{beta_a}.')
