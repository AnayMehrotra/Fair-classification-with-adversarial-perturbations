...
class AdversarialDebiasing(Transformer):
    ...
    def _classifier_model(self, features, features_dim, keep_prob):
        ...
        with tf.compat.v1.variable_scope("classifier_model"):
            W1 = tf.compat.v1.get_variable('W1', [features_dim, self.classifier_num_hidden_units],
                                  initializer=tf.contrib.layers.xavier_initializer(seed=self.seed1))
            b1 = tf.compat.v1.Variable(tf.zeros(shape=[self.classifier_num_hidden_units]), name='b1')

            h1 = tf.nn.relu(tf.matmul(features, W1) + b1)
            h1 = tf.nn.dropout(h1, rate=1-keep_prob, seed=self.seed2)

            W2 = tf.compat.v1.get_variable('W2', [self.classifier_num_hidden_units, 1],
                                 initializer=tf.contrib.layers.xavier_initializer(seed=self.seed3))
            b2 = tf.compat.v1.Variable(tf.zeros(shape=[1]), name='b2')
    ...
    def _adversary_model(self, pred_logits, true_labels):
        ...
        with tf.compat.v1.variable_scope("adversary_model"):
            ...
            s = tf.compat.v1.sigmoid((1 + tf.abs(c)) * pred_logits)

            W2 = tf.compat.v1.get_variable('W2', [3, 1],
    ...
    def fit(self, dataset):
        ...
        with tf.compat.v1.variable_scope(self.scope_name):
            ...
            self.true_labels_ph = tf.compat.v1.placeholder(tf.float32, shape=[None,1])
